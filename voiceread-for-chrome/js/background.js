...
chrome.runtime.onConnect.addListener(function(port) {
  ...
  port.onMessage.addListener(function(msg) { //the msg is sent by the chrome extension--contains info about the user's interaction(buttons pressed)
  	if (msg.type == "request") {
      chrome.tts.getVoices( function(voice_list) { // chrome.tts is textospeech. getVoices gets all voices
        port.postMessage({voices: voice_list.filter(isValidVoice)});  //how do we know its a valid voice?    
      });
